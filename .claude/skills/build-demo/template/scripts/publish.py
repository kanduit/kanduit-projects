# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable {{TITLE}} files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/{{SLUG}}/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/{{SLUG}}/        → what GitHub Pages actually serves
...
{{SLUG}}-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
SRC = os.path.dirname(HERE)                                  # portfolio/{{SLUG}}
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "{{SLUG}}")
...
def main():
    ...
    if changed:
        print("published to docs/{{SLUG}}/: " + ", ".join(changed))
    else:
