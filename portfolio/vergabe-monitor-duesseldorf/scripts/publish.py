# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable Vergabe-Monitor files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/vergabe-monitor-duesseldorf/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/vergabe-monitor-duesseldorf/        → what GitHub Pages actually serves
...
vergabe-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
Typical flow:  scripts/fetch_notices.py  →  scripts/generate.py  →
...
SRC = os.path.dirname(HERE)                                  # portfolio/vergabe-monitor-duesseldorf
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "vergabe-monitor-duesseldorf")
...
def main():
    ...
    if changed:
        print("published to docs/vergabe-monitor-duesseldorf/: " + ", ".join(changed))
    else:
