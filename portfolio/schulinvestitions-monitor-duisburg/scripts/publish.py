# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable Schulinvestitions-Monitor Duisburg files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/schulinvestitions-monitor-duisburg/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/schulinvestitions-monitor-duisburg/        → what GitHub Pages actually serves
...
schulinvestitions-monitor-duisburg-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
SRC = os.path.dirname(HERE)                                  # portfolio/schulinvestitions-monitor-duisburg
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "schulinvestitions-monitor-duisburg")
...
def main():
    ...
    if changed:
        print("published to docs/schulinvestitions-monitor-duisburg/: " + ", ".join(changed))
    else:
