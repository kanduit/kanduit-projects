...
Erzeugt zwei kleine, gefilterte Snapshots in data/sources/:
  ...
  msb_grundschulen_kr.json — die Grundschulen in Krefeld (Gemeindeschluessel
      05114000, Schulform 02, in Betrieb) mit Anschrift, Rechtsform,
      Schuelerzahl, Sozialindexstufe des Landes und WGS84-Koordinate.
  msb_zeitreihe_kr.json    — Grundschulen / Schueler / Klassen in Krefeld je
      Schuljahr 2012 ff.
...
AGS_KR = "05114000"      # Gemeindeschluessel Krefeld
KREIS_KR = "114"         # Kreisschluessel in der Zeitreihe
...
def main():
    ...
    gs = []
    for r in schulen:
        if r.get("Gemeindeschluessel") != AGS_KR:
            continue
    ...
    reihe = []
    for r in zeitreihe:
        if r.get("KREIS") != KREIS_KR or r.get("SCHULFORM_Text") != "Grundschule":
            continue
        reihe.append({
            "jahr": int(r["JAHR"]),
            "rechtsform": "privat" if r.get("RECHTSFORM") == "2" else "oeffentlich",
    ...
    if not reihe:
        raise SystemExit("Zeitreihe fuer Krefeld leer — Quelle pruefen")

    dump(os.path.join(OUT, "msb_grundschulen_kr.json"), {
        "meta": {
            ...
            "filter": "Gemeindeschluessel %s, Schulform 02 (Grundschule), "
                      "Schulbetrieb aktiv" % AGS_KR,
            "schuljahr": "2025/26",
            "abruf": stand,
            "hinweis": "Nur Einrichtungsdaten. Keine personenbezogenen Daten. "
                       "Die Sozialindexstufe ist die neunstufige Stufe des Landes "
                       "NRW — nicht der fuenfstufige kommunale "
                       "Grundschulsozialindex der Stadt Krefeld.",
        },
        "schulen": gs,
    })
    dump(os.path.join(OUT, "msb_zeitreihe_kr.json"), {
        "meta": {
            "quelle": "Ministerium fuer Schule und Bildung NRW — Open Data, "
                      "Schuelerzahlen nach Kreis und Schulform",
            "quelle_url": "https://www.schulministerium.nrw/open-data",
            "dateien": [URL_ZEITREIHE],
            "filter": "Kreis %s (Krfr. Stadt Krefeld), Schulform Grundschule" % KREIS_KR,
            "abruf": stand,
        },
        "reihe": reihe,
    })
