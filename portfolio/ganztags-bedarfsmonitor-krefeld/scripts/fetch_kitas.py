# -*- coding: utf-8 -*-
"""
fetch_kitas.py — Kindertageseinrichtungen in Krefeld aus dem offenen
Geodatenangebot des Landes NRW.

Quelle: https://www.opengeodata.nrw.de/produkte/bildung_wissenschaft/kitas/
        KiTasNRW_EPSG4326_CSV.csv

Zweck: Der Ue3-Platzbestand je Bezirk ist der Vorlauf-Indikator fuer die
kommenden Einschulungsjahrgaenge. Er wird in der Oberflaeche ausschliesslich
aggregiert je Stadtbezirk und Stadtteil gezeigt.
...
def main():
    ...
    path = os.path.join(OUT, "kitas_kr.json")
    with open(path, "w", encoding="utf-8") as fh:
        json.dump({
            "meta": {
                "quelle": "Kindertageseinrichtungen in NRW — Open Data NRW "
                          "(opengeodata.nrw.de)",
                "quelle_url": "https://www.opengeodata.nrw.de/produkte/"
                              "bildung_wissenschaft/kitas/",
                "dateien": [URL],
                "filter": "Ort = Krefeld",
                "abruf": datetime.date.today().isoformat(),
                "hinweis": ("Ohne Einrichtungsnamen, Traeger und Adressen — nur "
                            "Koordinate und Platzzahlen. Auswertung ausschliesslich "
                            "je Bezirk."),
            },
            "kitas": kitas,
        }, fh, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
        fh.write("\n")
    print("wrote", path, "(%d B, %d Einrichtungen)"
          % (os.path.getsize(path), len(kitas)))
