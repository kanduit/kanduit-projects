# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable Ganztags-Bedarfsmonitor Krefeld files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/ganztags-bedarfsmonitor-krefeld/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/ganztags-bedarfsmonitor-krefeld/        → what GitHub Pages actually serves
...
ganztags-bedarfsmonitor-krefeld-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
SRC = os.path.dirname(HERE)                                  # portfolio/ganztags-bedarfsmonitor-krefeld
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "ganztags-bedarfsmonitor-krefeld")
...
def main():
    ...
    if changed:
        print("published to docs/ganztags-bedarfsmonitor-krefeld/: " + ", ".join(changed))
    else:
