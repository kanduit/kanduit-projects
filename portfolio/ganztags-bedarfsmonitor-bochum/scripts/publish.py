# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable Ganztags-Bedarfsmonitor Bochum files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/ganztags-bedarfsmonitor-bochum/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/ganztags-bedarfsmonitor-bochum/        → what GitHub Pages actually serves
...
ganztags-bedarfsmonitor-bochum-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
SRC = os.path.dirname(HERE)                                  # portfolio/ganztags-bedarfsmonitor-bochum
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "ganztags-bedarfsmonitor-bochum")
...
def main():
    ...
    if changed:
        print("published to docs/ganztags-bedarfsmonitor-bochum/: " + ", ".join(changed))
    else:
