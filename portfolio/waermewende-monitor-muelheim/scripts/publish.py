# -*- coding: utf-8 -*-
"""
publish.py — copy the deployable Wärmewende-Monitor Mülheim files into the published
...
The repo keeps a deliberate split:
  ...
  portfolio/waermewende-monitor-muelheim/   → source of truth (also holds the
                                              fetch/generate scripts and the raw
                                              data snapshots — NOT for the public)
  docs/waermewende-monitor-muelheim/        → what GitHub Pages actually serves
...
waermewende-monitor-muelheim-publish-check.yml) runs it and fails if the committed docs/ copy is out
...
SRC = os.path.dirname(HERE)                                  # portfolio/waermewende-monitor-muelheim
REPO = os.path.dirname(os.path.dirname(SRC))                 # repo root
DEST = os.path.join(REPO, "docs", "waermewende-monitor-muelheim")
...
def main():
    ...
    if changed:
        print("published to docs/waermewende-monitor-muelheim/: " + ", ".join(changed))
    else:
